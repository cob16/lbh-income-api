...
describe "seeing letters' classification change after sync", type: :request do
  ...
  let(:original_env_can_auto_letters) { nil }
  let(:original_env_patch_codes_letters) { nil }
  let(:original_env_can_auto_letter_one) { nil }
  let(:original_env_can_auto_letter_two) { nil }
  ...
  before do
    ...
    original_env_can_auto_letters = ENV['CAN_AUTOMATE_LETTERS']
    original_env_patch_codes_letters = ENV['PATCH_CODES_FOR_LETTER_AUTOMATION']
    original_env_can_auto_letter_one = ENV['AUTOMATE_INCOME_COLLECTION_LETTER_ONE']
