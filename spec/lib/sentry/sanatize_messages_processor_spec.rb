...
describe SanatizeMessagesProcessor do
  ...
  context 'processor' do
    it { expect(data[:message]).to have_key(:sanatized) }
    it { expect(data[:variables]).to have_key(:sanatized) }
    it { expect(data[:phone_number]).to eq Raven::Processor::STRING_MASK }
    it { expect(data[:tenancy_ref]).to eq Raven::Processor::STRING_MASK }
    it { expect(data[:phone_number]).to eq Raven::Processor::STRING_MASK }
  end
